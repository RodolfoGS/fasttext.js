 ...
 * @copyright Copyright (c) 2017-2023 Loreto Parisi
*/
...
(function () {
    ...
    FastText = (function () {
        ...
        function FastText(options) {
            ...
            /**
             * Get Prob Vector from WASM model output
             * @param {*} predictions 
             * @param {*} limit 
             * @returns 
             */
            this.getProbs = function (predictions, limit) {
                limit = limit || Infinity;
                var probs = [];
                for (let i = 0; i < predictions.size() && i < limit; i++) {
                    let prediction = predictions.get(i);
                    let label = prediction[1].toString().replace(/__label__(.)/g, "$1").toUpperCase()
                    probs.push({ label: label, score: prediction[0] });
                }
                return probs;
            }//getProbs

            /**
             * Print Prob Vector from WASM model output
             * @param {*} predictions 
             * @param {*} limit 
             */
            this.printVector = function(predictions, limit) {
                limit = limit || Infinity;
                for (let i=0; i<predictions.size() && i<limit; i++){
                    let prediction = predictions.get(i);
        ...
        FastText.prototype.load = function () {
            var self = this;
            if(self._options.predict.wasm) return self.loadWASM();
            return new Promise(function (resolve, reject) {
         ...
         * @returns 
         */
        FastText.prototype.loadWASM = function () {
            var self = this;
            return new Promise(function (resolve, reject) {
                FastTextWasm.addOnPostRun(() => {
                    let ftwasm = new FastTextWasm.FastText();
                    ftwasm.loadModel(self._options.loadModel)
                        .then(model => {
                            self.model = model;
         ...
         * @returns 
         */
        FastText.prototype.predictWASM = function (data) {
            var self = this;
            return new Promise(function (resolve, reject) {
                try {
                    if (self._options.predict.normalize) { // normalize and send
                        data = FastText.normalize(data);
                        var predictions = self.model.predict(data,
                            self._options.predict.mostlikely,
                            self._options.predict.threshold);
                        return resolve(self.getProbs(predictions))
                    } else { // send raw
                        var predictions = self.model.predict(data,
                            self._options.predict.mostlikely,
                            self._options.predict.threshold);
         ...
         */
        FastText.prototype.loadSentence = function (value) {
            var self = this;
            return new Promise(function (resolve, reject) {
         ...
         */
        FastText.prototype.loadWord = function (value) {
            var self = this;
            return new Promise(function (resolve, reject) {
        ...
        FastText.prototype.predict = function (data = "") {
            var self = this;
            if(self._options.predict.wasm) return self.predictWASM(data);
            return new Promise(function (resolve, reject) {
                // register callbacks
                var onErrorCallback = function (error) {
                    return reject(error);
